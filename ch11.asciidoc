...
在Merkle证明中，你可以看到虚线框对应轻客户端计算的哈希。特别是，Merkle根是H~ABCDEFGHIJKLMNOP~，然后可以与已验证证明-of-work的块头进行比较。
...
全节点可以发送有关块和轻客户端可以重新计算Merkle根的有限信息，然后将其与块头中的Merkle根进行验证。 这不能保证交易在最长区块链中，但确实让轻客户端知道全节点必须花费大量哈希能力或能量创建有效的证明-of-work。 只要创建这种证明-of-work的奖励大于交易中的金额，轻客户端至少可以知道全节点没有明确的欺诈动机。
...
由于块头可以从多个节点请求，因此轻客户端有一种方法可以验证是否有一个节点试图显示不是最长的块头。 只需要一个诚实节点来使一百个不诚实的节点无效，因为证明-of-work是客观的。 因此，要欺骗轻客户端（即控制轻客户端连接的对象），需要将其隔离。 SPV的安全性要求网络中存在大量诚实节点。
...
换句话说，轻客户端的安全性基于一个强大的节点网络和产生证明-of-work的经济成本。 对于与块补贴（目前为12.5 BTC）相比较小的交易，可能无需担心。 对于大型交易（例如100 BTC），则全节点可能有欺诈您的经济动机。 这种规模的交易通常应使用全节点进行验证。
